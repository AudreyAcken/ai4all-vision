...
#     # YOUR CODE HERE
#     for a in range(len(data_batch)):
#         avg = np.mean(learning_rate*data_batch[a]*(labels_batch[a]-predict_probability(data_batch[a],weights)))
#         weights += avg
#         weights -= regularization_rate*weights
#     return weights
    ...
    for i in range(len(data_batch)):
        ...
        weights += learning_rate*(np.mean(data_batch[i]*(labels_batch[i]-prob), axis=0))
        weights -= regularization_rate*weights
    return weights
